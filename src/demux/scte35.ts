...
export type SCTE35Data =
	...
	| {
			splice_command_type:
				| SCTE35CommandType.kSpliceNull
				| SCTE35CommandType.kBandwidthReservation
				| SCTE35CommandType.kSpliceSchedule
				| SCTE35CommandType.kPrivateCommand;
			pts: undefined;
			nearest_pts?: number;
			detail: SCTE35Detail;
			data: Uint8Array;
...
type SpliceNull = Record<string, never>;
type SpliceSchedule = {
	splice_count: number;
...
type BandwidthReservation = Record<string, never>;
type PrivateCommand = {
	identifier: string;
...
const parseSegmentationDescriptor = (
	descriptor_tag: number,
	descriptor_length: number,
	identifier: string,
	reader: ExpGolomb,
...
export const readSCTE35 = (data: Uint8Array): SCTE35Data => {
	...
	if (splice_command_type === SCTE35CommandType.kSpliceInsert) {
		...
		if (spliceInsert.splice_event_cancel_indicator) {
			...
			const duraiton = spliceInsert.duration_flag ? (spliceInsert.break_duration?.duration ?? 0) / 90 : undefined;
