...
class TSDemuxer extends BaseDemuxer {
    ...
    while (offset + this.ts_packet_size_ <= chunk.byteLength) {
      ...
      if (
        ...
        if (
          ...
          (this.pmt_ != undefined &&
            this.pmt_.pid_stream_type[pid] === StreamType.kSCTE35)
        ) {
          // SCTE35
      ...
      if (stream_type === StreamType.kH264 && !already_has_video) {
        ...
        if (ES_info_length > 0) {
          ...
          for (let offset = i + 5; offset < i + 5 + ES_info_length; ) {
            ...
            if (tag === 0x26) {
              let metadata_application_format =
              ...
              let metadata_format =
                data[
                  offset + 4 + (metadata_application_format === 0xffff ? 4 : 0)
                ];
  ...
  private parseH264Payload(
    ...
    while ((nalu_payload = annexb_parser.readNextNaluPayload()) != null) {
      ...
      if (nalu_avc1.type === H264NaluType.kSliceSPS) {
        ...
        nalu_avc1.type === H264NaluType.kSliceNonIDR &&
  ...
  private dispatchVideoInitSegment() {
    ...
    if (this.video_metadata_.av1c) {
      ...
      let hvcc = new HEVCDecoderConfigurationRecord(
        vps_without_header,
        sps_without_header,
        pps_without_header,
        details
  ...
  private parseMP3Payload(data: Uint8Array, pts: number) {
    ...
    let _mpegAudioL1BitRateTable = [
      0, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448, -1,
    ];
    let _mpegAudioL2BitRateTable = [
      0, 32, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, -1,
    ];
    let _mpegAudioL3BitRateTable = [
  ...
  private dispatchAudioInitSegment(sample: AudioData) {
    ...
    if (this.audio_metadata_.codec === "aac") {
      ...
      meta.refSampleDuration =
        ((256 * ec3_config.num_blks) / meta.audioSampleRate) * meta.timescale; // TODO: blk size
