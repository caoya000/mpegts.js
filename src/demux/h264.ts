...
export class AVCDecoderConfigurationRecord {
	...
	public constructor(sps: Uint8Array, pps: Uint8Array, sps_details: Record<string, unknown>) {
		...
		if (need_extra_fields) {
			data[offset] = 0xfc | (sps_details.chroma_format_idc as number);
			data[offset + 1] = 0xf8 | ((sps_details.bit_depth_luma as number) - 8);
			data[offset + 2] = 0xf8 | ((sps_details.bit_depth_chroma as number) - 8);
