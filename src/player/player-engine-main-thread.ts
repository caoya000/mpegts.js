...
interface MainThreadEventHandlers {
...
class PlayerEngineMainThread implements PlayerEngine {
	...
	public attachMediaElement(mediaElement: HTMLMediaElement): void {
		...
		mediaElement.addEventListener("loadedmetadata", this.e?.onMediaLoadedMetadata as EventListener);
		...
		this._mse_controller.initialize({
			getCurrentTime: () => this._media_element?.currentTime ?? 0,
			getReadyState: () => this._media_element?.readyState ?? 0,
		});
		...
		if (this._mse_controller.isManagedMediaSource()) {
			...
			mediaElement.srcObject = this._mse_controller.getObject() as unknown as MediaSource;
		} else {
			// w3c MediaSource
	...
	public load(): void {
		...
		this._transmuxer.on(TransmuxingEvents.INIT_SEGMENT, ((_type: unknown, is: unknown) => {
			this._mse_controller?.appendInitSegment(is as { type: string; container: string; [key: string]: unknown });
		}) as (...args: unknown[]) => void);
		this._transmuxer.on(TransmuxingEvents.MEDIA_SEGMENT, ((type: unknown, ms: unknown) => {
			const segment = ms as Record<string, unknown>;
			this._mse_controller?.appendMediaSegment(segment as { type: string; [key: string]: unknown });
			if (
				!this._config.isLive &&
				type === "video" &&
				segment.data &&
				(segment.data as ArrayBuffer).byteLength > 0 &&
				"info" in segment
			) {
				this._seeking_handler?.appendSyncPoints(
					(segment.info as Record<string, unknown>).syncPoints as import("../core/media-segment-info").SampleInfo[],
				);
			}
			this._loading_controller?.notifyBufferedPositionChanged(
				((segment.info as Record<string, unknown>).endDts as number) / 1000,
			);
		}) as (...args: unknown[]) => void);
		...
		this._transmuxer.on(TransmuxingEvents.MEDIA_INFO, (mediaInfo: unknown) => {
			this._media_info = mediaInfo as MediaInfo;
			this._emitter?.emit(PlayerEvents.MEDIA_INFO, Object.assign({}, mediaInfo));
		});
		this._transmuxer.on(TransmuxingEvents.STATISTICS_INFO, (statInfo: unknown) => {
			this._statistics_info = this._fillStatisticsInfo(statInfo as Record<string, unknown>);
			this._emitter?.emit(PlayerEvents.STATISTICS_INFO, Object.assign({}, statInfo));
		});
		this._transmuxer.on(TransmuxingEvents.RECOMMEND_SEEKPOINT, (milliseconds: unknown) => {
		...
		this._transmuxer.on(TransmuxingEvents.PGS_SUBTITLE_ARRIVED, (pgs_data: unknown) => {
			this._emitter?.emit(PlayerEvents.PGS_SUBTITLE_ARRIVED, pgs_data);
		});
		this._transmuxer.on(TransmuxingEvents.SYNCHRONOUS_KLV_METADATA_ARRIVED, (synchronous_klv_metadata: unknown) => {
			this._emitter?.emit(PlayerEvents.SYNCHRONOUS_KLV_METADATA_ARRIVED, synchronous_klv_metadata);
		});
		this._transmuxer.on(TransmuxingEvents.ASYNCHRONOUS_KLV_METADATA_ARRIVED, (asynchronous_klv_metadata: unknown) => {
			this._emitter?.emit(PlayerEvents.ASYNCHRONOUS_KLV_METADATA_ARRIVED, asynchronous_klv_metadata);
		});
		this._transmuxer.on(TransmuxingEvents.SMPTE2038_METADATA_ARRIVED, (smpte2038_metadata: unknown) => {
			this._emitter?.emit(PlayerEvents.SMPTE2038_METADATA_ARRIVED, smpte2038_metadata);
		});
		this._transmuxer.on(TransmuxingEvents.SCTE35_METADATA_ARRIVED, (scte35_metadata: unknown) => {
			this._emitter?.emit(PlayerEvents.SCTE35_METADATA_ARRIVED, scte35_metadata);
		});
		this._transmuxer.on(TransmuxingEvents.PES_PRIVATE_DATA_DESCRIPTOR, (descriptor: unknown) => {
			this._emitter?.emit(PlayerEvents.PES_PRIVATE_DATA_DESCRIPTOR, descriptor);
		});
		this._transmuxer.on(TransmuxingEvents.PES_PRIVATE_DATA_ARRIVED, (private_data: unknown) => {
	...
	private _onMSEError(info: unknown): void {
