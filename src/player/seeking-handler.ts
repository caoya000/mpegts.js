...
class SeekingHandler {
	...
	private _idr_sample_list: IDRSampleList | null = new IDRSampleList();
	...
	public constructor(
		...
		const chrome_need_idr_fix =
			Browser.chrome &&
			((Browser.version?.major ?? 0) < 50 ||
				((Browser.version?.major ?? 0) === 50 && (Browser.version?.build ?? 0) < 2661));
		this._always_seek_keyframe = !!(chrome_need_idr_fix || Browser.msedge || Browser.msie);
	...
	public seek(seconds: number): void {
		...
		if (seconds < 1.0 && this._media_element && this._media_element.buffered.length > 0) {
	...
	public appendSyncPoints(syncpoints: SampleInfo[]): void {
	...
	private _onMediaSeeking(_e: Event): void {
		...
		if (target < 1.0 && buffered.length > 0) {
			...
			if ((video_begin_time < 1.0 && target < video_begin_time) || Browser.safari) {
				...
				const target: number = Browser.safari ? 0.1 : video_begin_time;
	...
	private _pollAndApplyUnbufferedSeek(): void {
		...
		if (record_time <= SeekingHandler._getClockTime() - 100) {
			const target = this._media_element?.currentTime ?? 0;
	...
	private _getNearestKeyframe(dts: number): SampleInfo | null {
		return this._idr_sample_list?.getLastSyncPointBeforeDts(dts) ?? null;
	}
